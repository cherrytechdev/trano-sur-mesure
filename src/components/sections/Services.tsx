...
const textureMap: Record<string, string> = {
  ...
  suivi:
    "bg-[repeating-linear-gradient(0deg,transparent,transparent_39px,var(--color-line)_39px,var(--color-line)_40px)] opacity-[0.1]",
};

const iconBoxVariants: Record<string, string> = {
  conception: "bg-laterite shadow-sm group-hover:shadow-md",
  realisation:
    "bg-gradient-to-br from-laterite to-laterite-dark shadow-md group-hover:shadow-lg",
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
          {SERVICES.map((service) => (
            <div key={service.title} className="relative group">
              ...
              <div
                className="hidden md:block absolute -top-2 -left-2 w-10 h-10 pointer-events-none"
                aria-hidden
              >
              ...
              <div
                className="hidden md:block absolute -bottom-2 -right-2 w-10 h-10 pointer-events-none"
                aria-hidden
              >
              ...
              >
                ...
                <div className="relative z-10 flex flex-col h-full">
                  ...
                  <span className="self-start inline-flex items-center gap-2 bg-parchment text-laterite font-sans font-bold text-[9px] uppercase tracking-widest px-4 py-2 ring-1 ring-ink/5 group-hover:bg-laterite group-hover:text-parchment group-hover:ring-laterite transition-all duration-500">
                    {service.variant === "conception" && (
                      <span className="w-1.5 h-1.5 bg-laterite/40 rounded-full group-hover:bg-parchment/40 transition-colors duration-500" />
                    )}
                    {service.variant === "realisation" && (
                      <span className="w-3 h-px bg-laterite/40 group-hover:bg-parchment/40 transition-colors duration-500" />
                    )}
                    {service.variant === "suivi" && (
                      <span className="w-1.5 h-1.5 border border-laterite/40 rotate-45 group-hover:border-parchment/40 transition-colors duration-500" />
                    )}
                    {service.tag}
                  </span>
                </div>
